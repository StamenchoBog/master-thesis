...
- **Protocol discipline:** cooldown gate (SoC ≤ 40°C stable) before every measured run; A/B order counterbalanced; N=5 paired seeds; Wilcoxon signed-rank + Cliff's delta; parameters frozen in `experiments/protocol.md` after the pilot, before measured runs.
